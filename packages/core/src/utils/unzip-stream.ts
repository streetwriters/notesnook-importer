...
export class Entry extends Blob {
  private dataView: DataView;
  private _fileLike: Blob;
  private _extraFields: Record<string, DataView>;

  constructor(dataView: DataView, fileLike: Blob) {
    super();
  ...
  async arrayBuffer() {
    ...
    let read = 0;
    for await (const chunk of toAsyncIterator(this.stream())) {
      data.set(chunk, read);
  ...
  slice(start?: number, end?: number, contentType?: string): Blob {
    return this._fileLike.slice(start, end, contentType);
  }
}
...
export async function unzip(zip: IFile): Promise<IFile[]> {
  ...
  const { read } = await Reader(zip.data);
