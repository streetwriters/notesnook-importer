...
/**
 * The data for a {@link FileNode} structure (section 2.4.3) that specifies an entry in the current global identification table (section 2.1.3) by referring to an entry in the global identification table of the dependency revision (section 2.1.8) of the revision manifest (section 2.1.9) that contains this global identification table. The value of the FileNode.FileNodeID field MUST be 0x025.
 */
export type GlobalIdTableEntry2FNDX = {
  /**
   * An unsigned integer that specifies the index of the entry in the dependency revision’s global identification table that is used to define this entry. The index MUST be present in the global identification table of the dependency revision (section 2.1.8).
   */
  iIndexMapFrom: number;

  /**
   * An unsigned integer that specifies the index of the entry in the current global identification table. MUST be less than 0xFFFFFF. MUST be unique relative to the other indices in this global identification table specified by FileNode structures with the value of the FileNode.FileNodeID field equal to 0x024 (GlobalIdTableEntryFNDX structure), 0x25 (GlobalIdTableEntry2FNDX structure), and 0x26 (GlobalIdTableEntry3FNDX structure).
   */
  iIndexMapTo: number;
};

export function GlobalIdTableEntry2FNDX(
  reader: OneNoteReader
): GlobalIdTableEntry2FNDX {
  return {
    iIndexMapFrom: reader.deserializeInt(),
    iIndexMapTo: reader.deserializeInt(),
  };
}
